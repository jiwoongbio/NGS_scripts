...
my ($inputFile, @rankList) = @ARGV;
my %rankHash = map {$_ => 1} @rankList;
open(my $reader, ($inputFile =~ /\.gz$/ ? "gzip -dc $inputFile |" : $inputFile)) or die "Can't open '$inputFile': $!";
while(my $line = <$reader>) {
	chomp($line);
	my @tokenList = split(/\t/, $line, -1);
	my $taxonId = $tokenList[$index];
	my $rankTaxonId = getRankTaxonId($taxonId);
	print join("\t", @tokenList, $rankTaxonId), "\n" if($withRankTaxonIdOnly eq '' || $rankTaxonId ne '');
}
close($reader);
