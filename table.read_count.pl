...
foreach my $sample (@sampleList) {
	...
	if(-s "$sample/$sample.read_length.read_count.txt") {
		addColumn('Read length', 'Total reads');
		chomp($columnSampleCountHash{'Read length'}->{$sample} = `cut -f2 $sample/$sample.read_length.read_count.txt | sort -nr | head -n1`);
		chomp($columnSampleCountHash{'Total reads'}->{$sample} = `cut -f3 $sample/$sample.read_length.read_count.txt | awk '{a += \$o} END {print a}'`);
	}
	if(-s "$sample/$sample.trimgalore.read_length.read_count.txt") {
		addColumn('QC-passed reads', '% QC-passed reads');
		chomp($columnSampleCountHash{'QC-passed reads'}->{$sample} = `cut -f3 $sample/$sample.trimgalore.read_length.read_count.txt | awk '{a += \$o} END {print a}'`);
	...
	if(-s (my $file = "$sample/$sample.transcriptome.remocon.log")) {
		chomp(my @typeCountList = `cat $file`);
		my %typeCountHash = map {$_->[0] => $_->[1] * 2} map {[split(/: /, $_, 2)]} @typeCountList;
	...
	if(-s "$sample/$sample.transcriptome.remocon.total_read_count.txt") {
		chomp(my $count = `cat $sample/$sample.transcriptome.remocon.total_read_count.txt`);
		print STDERR join("\t", $sample, 'different read count', 'transcriptome'), "\n" if($count != $columnSampleCountHash{'Non-contaminant transcriptome reads'}->{$sample});
	}
	if(-s "$sample/$sample.transcriptome.total_read_count.txt") {
		chomp(my $count = `cat $sample/$sample.transcriptome.total_read_count.txt`);
		print STDERR join("\t", $sample, 'different read count', 'transcriptome'), "\n" if($count != $columnSampleCountHash{'QC-passed reads'}->{$sample});
	}
	if(my ($file) = grep {-s $_} ("$sample/$sample.transcriptome.mapping_strand.read_count.txt", "$sample/$sample.transcriptome.remocon.mapping_strand.read_count.txt")) {
	...
	if(my ($file) = grep {-s $_} ("$sample/$sample.transcriptome.insert_size_metrics.txt", "$sample/$sample.transcriptome.remocon.insert_size_metrics.txt")) {
	...
	if(-s "$sample/$sample.rRNA.read_count.txt") {
		addColumn('rRNA-mapped reads', '% rRNA-mapped reads');
		chomp($columnSampleCountHash{'rRNA-mapped reads'}->{$sample} = `cat $sample/$sample.rRNA.read_count.txt`);
	...
	if(my ($file) = grep {-s $_} ("$sample/$sample.STAR/Log.final.out", "$sample/$sample.STAR.hg38/Log.final.out")) {
		chomp(my @typeCountList = `cat $file`);
	...
	if(-s "$sample/$sample.genome.total_read_count.txt") {
		chomp(my $count = `cat $sample/$sample.genome.total_read_count.txt`);
		print STDERR join("\t", $sample, 'different read count', 'genome'), "\n" if($count != $columnSampleCountHash{'QC-passed reads'}->{$sample});
	}
	if(-s "$sample/$sample.genome.mapped_read_count.txt") {
		addColumn('Genome-mapped reads', '% Genome-mapped reads');
		chomp($columnSampleCountHash{'Genome-mapped reads'}->{$sample} = `cat $sample/$sample.genome.mapped_read_count.txt`);
		$columnSampleCountHash{'% Genome-mapped reads'}->{$sample} = sprintf('%.1f%%', $columnSampleCountHash{'Genome-mapped reads'}->{$sample} / $columnSampleCountHash{'QC-passed reads'}->{$sample} * 100);
	}
	if(-s "$sample/$sample.genome.uniquely_mapped_read_count.txt") {
		addColumn('Uniquely mapped reads', '% Uniquely mapped reads');
		chomp($columnSampleCountHash{'Uniquely mapped reads'}->{$sample} = `cat $sample/$sample.genome.uniquely_mapped_read_count.txt`);
		$columnSampleCountHash{'% Uniquely mapped reads'}->{$sample} = sprintf('%.1f%%', $columnSampleCountHash{'Uniquely mapped reads'}->{$sample} / $columnSampleCountHash{'Genome-mapped reads'}->{$sample} * 100);
	}
	if(my ($file) = grep {-s $_} ("$sample/$sample.remocon.log", "$sample/$sample.STAR.remocon.log")) {
		chomp(my @typeCountList = `cat $file`);
	...
	if(my ($file) = grep {-s $_} ("$sample/$sample.dupMarked.metrics", "$sample/$sample.remocon.dupMarked.metrics")) {
		my $count = 0;
		my @columnList = ();
		open(my $reader, $file);
	...
	if(-s "$sample/$sample.target.read_count.txt") {
		addColumn('Target reads');
		chomp($columnSampleCountHash{'Target reads'}->{$sample} = `cat $sample/$sample.target.read_count.txt`);
	...
	if(-s "$sample/$sample.target.depth.txt") {
		chomp(my @typeCountList = `cat $sample/$sample.target.depth.txt`);
